...
exports.loginClient = async (username, password) => {
    ...
    try {
        ...
        const clientResult = await pool.query("SELECT * FROM clients WHERE client_username = $1", [username]);
        ...
        if (client.client_username !== username && client.client_password !== password) {
            throw new Error("Invalid username or password.");
        }
