const { pool, createAdminClient, createClientDbConnection } = require("../db/db");
...
exports.addClient = async (username, password, databaseName) => {
    ...
    try {
        ...
        clientDbConnection = createClientDbConnection(databaseName);
...
exports.getClientByUsername = async (username) => {
    const result = await pool.query("SELECT * FROM clients WHERE client_username = $1", [username]);
    return result.rows[0];
};

exports.updateClientPassword = async (username, password) => {
    await pool.query("UPDATE clients SET client_password = $1 WHERE client_username = $2", [password, username]);
...
exports.deleteClient = async (username) => {
    ...
    try {
        ...
        await pool.query("DELETE FROM clients WHERE client_username = $1", [username]);
